...
def detalle_completo_inmobiliaria():
    """
    Obtiene el resumen de las cuentas x cobrar de corporativo
    """
    ...
    proc_almacenado = "spA_Activos_Ocupacion_Detalle"

    c.callproc(proc_almacenado, (4, 0))
    
    ocupacion_detalle = c.fetchall()

    # print(ocupacion_detalle)

    c.close()
    conn.close()

    # DataFrame Ocupacion Activos
    df_ocupacion_detalle = pd.DataFrame.from_records(ocupacion_detalle)
    
    df_ocupacion_detalle.columns = ['ClaveTipo', 'Tipo', 'ClaveActivo', 'Activo1', 'Activo', 'Empresa', 'Propietario', 'NumCliente', 'Cliente', 'NumContrato', 'VigenciaDesde', 'Vigencia', 'Monto', 'Periodicidad']

    # Eliminar columnas que no necesitamos
    df_ocupacion_detalle.drop(['ClaveTipo'], axis=1, inplace=True)
    df_ocupacion_detalle.drop(['ClaveActivo'], axis=1, inplace=True)
    df_ocupacion_detalle.drop(['Activo1'], axis=1, inplace=True)
    df_ocupacion_detalle.drop(['Empresa'], axis=1, inplace=True)
    df_ocupacion_detalle.drop(['NumCliente'], axis=1, inplace=True)
    df_ocupacion_detalle.drop(['NumContrato'], axis=1, inplace=True)
    df_ocupacion_detalle.drop(['VigenciaDesde'], axis=1, inplace=True)

    # Convierte las columnas al tipo datetime
    df_ocupacion_detalle['Vigencia'] = pd.to_datetime(df_ocupacion_detalle['Vigencia'])

    # Cambia el formato de las columnas 'Vigencia' en el mismo DateFrame
    df_ocupacion_detalle[['Vigencia']] = df_ocupacion_detalle[['Vigencia']].applymap(lambda x: x.strftime('%d/%m/%y'))

    return df_ocupacion_detalle